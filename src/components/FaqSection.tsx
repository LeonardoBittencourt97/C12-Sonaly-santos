...
import { ChevronDown, HelpCircle, MessageSquare, ShieldAlert, HeartHandshake, Briefcase, Scale } from "lucide-react";
...
export function FaqSection() {
  ...
  const [activeTab, setActiveTab] = useState<string>("penal");
  ...
  const getSpecificQuestionUrl = (question: string) => {
    const text = `Olá, Dra. Sonaly! Estive lendo a dúvida "${question}" no seu site e gostaria de esclarecer sobre o meu caso. Poderia me orientar?`;
    return `https://wa.me/${OFFICE_INFO.whatsappNumber}?text=${encodeURIComponent(text)}`;
  };

  const getGeneralFaqUrl = () => {
    const text = `Olá, Dra. Sonaly! Minha dúvida não está listada nas perguntas frequentes do site. Gostaria de uma orientação jurídica para o meu caso.`;
    return `https://wa.me/${OFFICE_INFO.whatsappNumber}?text=${encodeURIComponent(text)}`;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="font-body text-sm sm:text-base text-[var(--text-muted)] max-w-xl leading-relaxed">
            Esclarecimentos diretos sobre Direito Penal, Família, Sucessões, Direito do Trabalho e rotinas do atendimento jurídico.
          </p>
        </div>
        ...
        >
          <div className="space-y-1 text-center sm:text-left">
            ...
            <p className="font-body text-xs sm:text-sm text-[var(--text-muted)] leading-relaxed">
              Cada situação jurídica possui particularidades. Envie sua pergunta diretamente para a Dra. Sonaly Cristina dos Santos.
            </p>
          </div>
