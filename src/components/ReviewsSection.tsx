    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        ...
        >
          ...
          {/* Badge sem número total conforme solicitado */}
          <div className="flex items-center gap-4 p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-subtle)]/35 shadow-2xs">
            ...
            <div className="font-heading text-sm sm:text-base font-bold text-[var(--text-main)]">
              Opiniões Verificadas <span className="block text-xs text-[var(--text-muted)] font-normal">no Google Brasil</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div ref={marqueeRef} className="w-full overflow-hidden py-4 will-change-transform">
        <div className="animate-marquee gap-6">
          {duplicatedReviews.map((rev, idx) => (
            <div
              key={`${rev.id}-${idx}`}
              className="w-[320px] sm:w-[380px] p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)]/35 shadow-xs flex flex-col justify-between flex-shrink-0 hover:border-[var(--brand-gold)] transition-colors"
            >
              <div>
              ...
              <div className="pt-3 border-t border-[var(--border-subtle)]/20 flex items-center justify-between text-xs font-heading">
                <div>
                  ...
                  {rev.role && (
                    <span className="text-[0.6875rem] text-[var(--text-muted)] font-body block">{rev.role}</span>
                  )}
                </div>
                <span className="text-[0.6875rem] text-[var(--brand-brown)] dark:text-[var(--brand-gold)] flex items-center gap-1 font-semibold">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <p className="text-[0.6875rem] text-[var(--text-muted)] text-center font-body">
          * Avaliações espontâneas e públicas registradas na plataforma Google Maps / Google Reviews, em observância à sobriedade exigida pelo Código de Ética e Disciplina da OAB.
        </p>
      </div>
    </section>
  );
